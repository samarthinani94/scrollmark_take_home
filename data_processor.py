...
class DataProcessor:
    ...
    def _clean_caption_text(self, text):
        """Cleans caption text, preserving @mentions and removing stop words."""
        ...
        # Correctly filter for alphabetic words or mentions, AND remove stop words.
        cleaned_tokens = [
    ...
    def _clean_comment_text(self, text):
        """Cleans comment text, standardizing @mentions and removing stop words."""
        if not isinstance(text, str): return ""
        text = emoji.replace_emoji(text, replace='')
        text = text.lower()
